...
class WebsiteSettingController extends Controller
{
    ...
    public function website_setting()
    {
        try {
            ...
            // Always retrieve English content by default
            $exist_cms_content = $cms_content
                ->where('page_slug', 'terms_condition_page')
                ->where('translation_code', session()->get('terms_condition_page', 'en'))
                ->first();
            ...
            $exist_cms_content_privacy = $cms_content
                ...
                ->where('translation_code', session()->get('privacy_page', 'en'))
                ->first();
            if ($exist_cms_content_privacy) {
                $privacy_page = $exist_cms_content_privacy->text;
            }
    
            $exist_cms_content_refund = $cms_content
                ->where('page_slug', 'refund_page')
                ->where('translation_code', session()->get('refund_page', 'en'))
                ->first();
            if ($exist_cms_content_refund) {
                $refund_page = $exist_cms_content_refund->text;
            }
    
            return view('backend.settings.pages.website_setting', compact('about_list', 'languages', 'terms_page', 'privacy_page', 'refund_page', 'privacy_page_list', 'terms_condition_page_list', 'refund_page_list'));
        } catch (\Exception $e) {
